...
export default function CattedraMaterial() {
  ...
  const handleGenerateQuestions = () => {
    generateQuestions.mutate({ id: materialId }, {
      onSuccess: () => {
        toast({
          title: "Bozze pronte da rivedere",
          description: "L'assistente ha preparato una prima stesura: rivedila e approva ciò che convince.",
        });
      ...
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Le tue domande ({material.questionCount})</CardTitle>
                <CardDescription>Rivedi, correggi o aggiungi le tue: agli studenti arrivano solo quelle approvate.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {questionsLoading ? (
                  <Skeleton className="h-32" />
                ) : !questions?.length ? (
                  <div className="text-sm text-muted-foreground">Nessuna domanda ancora.</div>
                ) : (
                  <>
                    {draftCount > 0 && (
                      <div className="rounded-lg border border-amber-300/60 bg-amber-50/60 px-3 py-2 text-sm text-amber-900">
                        {draftCount} {draftCount === 1 ? "bozza da rivedere" : "bozze da rivedere"}: non ancora visibili
                        agli studenti.
                      </div>
                    )}
                      ...
                      {questions.slice(0, 6).map((q) => (
                        <div key={q.id} className="text-sm border-b pb-3 last:border-0 last:pb-0">
                          ...
                          <div className="flex gap-2 mt-2">
                            ...
                            <span className="px-1.5 py-0.5 bg-primary/10 text-primary rounded text-[10px] uppercase truncate">
                              {q.topic}
                            </span>
                          </div>
                        </div>
                ...
                <Button asChild variant="outline" className="w-full">
                  <Link href={`/cattedra/material/${materialId}/domande`}>
